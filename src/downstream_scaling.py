...
def main(params: DictConfig) -> None:
    ...
    fraction_sample = params["fraction_sample"]
    ...
    test_subjects = feature_path / f"test_set_connectome_sessions-{n_sessions}.txt"
    # data file for this n
    data_file = f"inputs/connectomes/sourcedata/hcp_h5/hcp_{n_sessions}.h5"
    ...
    log.info(f"Predicting {params['predict_variable']}, using {n_sessions} sessions.")
    ...
    for measure in baseline_details:
        ...
        for k, (tng, tst) in enumerate(cv.split(dataset["data"], dataset["label"], groups=groups)):
            ...
            # fraction of training subjects
            n_tng = round(len(tng)*fraction_sample)
    ...
    with open(
        output_dir / f"simple_classifiers_{params['predict_variable']}_sessions-{n_sessions}_splits-{k_splits}_fraction-{fraction_sample}.json",
        "w",
    ) as f:
    ...
    baselines_df.to_csv(
        output_dir / f"simple_classifiers_{params['predict_variable']}_sessions-{n_sessions}_splits-{k_splits}_fraction-{fraction_sample}.tsv",
        sep="\t",
    )
